function renderGanttChart(scheduledProcesses) {
  ...
  scheduledProcesses.forEach((process, index) => {
    const startX = process.startTime * 10; // Scale for better visibility
    const width = (process.endTime - process.startTime) * 10; // Scale for better visibility
    ...
    // Draw process ID
    ctx.fillStyle = "#000";
    ctx.fillText(`P${process.processId}`, startX + 5, y + 20);
  });
}